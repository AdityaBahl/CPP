//#include <iostream>                   //same as 1.) maps, but in unordered, it saves time complexity(O(1) instead of O(N)) and presents list in no order
//#include <map>                        // some things cant be used in unordered maps like vectors and maps
#include <bits/stdc++.h>
...
{                    // if key repeated, latest value will be kept, others replaced
    ...
    auto iter = m.find(2); // to find a value(by key)
    if (iter == m.end())
        cout << "No Value" << endl;
    else
